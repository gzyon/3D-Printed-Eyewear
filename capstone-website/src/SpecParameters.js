...
// all the math
function SpecParameters(front, left, right, scale, setFrameWeight) {
    ...
    function getDensity(geometry, mass) {
        geometry.computeBoundingBox()
        const volume = (geometry.boundingBox.max.x - geometry.boundingBox.min.x) * (geometry.boundingBox.max.y - geometry.boundingBox.min.y) * (geometry.boundingBox.max.z - geometry.boundingBox.min.z);
        const density = mass / volume;
        return density; 
    }

    // nose bridge positions (hardcoded)
    const noseBridgePositions = {1: [front.position[0], front.position[1] - 5, front.position[2]], 2: [front.position[0], front.position[1] - 7, front.position[2]], 3: [front.position[0], front.position[1] - 7, front.position[2]]};
    ...
    const noseBridgePos = [noseBridgePositions[front.frameId][0], noseBridgePositions[front.frameId][1], noseBridgePositions[front.frameId][2] + (leftHingeOffset + rightHingeOffset) / 2];
    ...
    const leftArmEndRenderPoint = [right.position[0], right.position[1] - leftEndHeight - 2.5, right.position[2] - leftEndLength]; 
    ...
    y = (leftCenterRenderPoint[1] - rightCenterRenderPoint[1]);
    x = (leftCenterRenderPoint[0] - rightCenterRenderPoint[0]);
    z = (leftCenterRenderPoint[2] - rightCenterRenderPoint[2]);
    theta = Math.atan(y/x);
    phi = Math.atan(z/x);
    ...
    // calculating weight of specs
    const frontDensity = getDensity(front_geom, 6.8),
          sideCenterDensity = getDensity(leftCenter_geom, 3);
        //   sideEndDensity = getDensity(leftEnd_geom, 2);
    
    const frontFrameWeight = frontDensity * (front_geom.boundingBox.max.x - front_geom.boundingBox.min.x) * ((front_geom.boundingBox.max.y  - front_geom.boundingBox.min.y) * scale.frontScale[0]) * ((front_geom.boundingBox.max.z  - front_geom.boundingBox.min.z) * scale.frontScale[1]);
    const leftArmWeight = sideCenterDensity * ((leftCenter_geom.boundingBox.max.x - leftCenter_geom.boundingBox.min.x) * scale.leftScale) * (leftCenter_geom.boundingBox.max.y - leftCenter_geom.boundingBox.min.y) * (leftCenter_geom.boundingBox.max.z - leftCenter_geom.boundingBox.min.z);
    const rightArmWeight = sideCenterDensity * ((rightCenter_geom.boundingBox.max.x - rightCenter_geom.boundingBox.min.x) * scale.rightScale) * (rightCenter_geom.boundingBox.max.y - rightCenter_geom.boundingBox.min.y) * (rightCenter_geom.boundingBox.max.z - rightCenter_geom.boundingBox.min.z);
    console.log("total frame weight", frontFrameWeight + leftArmWeight + rightArmWeight + 4);
    setFrameWeight(frontFrameWeight + leftArmWeight + rightArmWeight + 4);
